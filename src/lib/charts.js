...
export function lineChart(series, years) {
  ...
  return `<svg viewBox="0 0 ${w} ${h}" width="100%" height="${h}" style="display:block">
    ...
    ${labels.map(v => `<line x1="${pad}" x2="${w - pad}" y1="${ys(v).toFixed(1)}" y2="${ys(v).toFixed(1)}" stroke="rgba(36,30,25,.14)" stroke-width="1" ${v > 0 ? 'stroke-dasharray="2 3"' : ''}/>
      <text x="${pad - 6}" y="${(ys(v) + 3).toFixed(1)}" fill="#8a7f74" font-size="10" font-family="'Geist Mono',ui-monospace" text-anchor="end">${de0.format(Math.round(v / 1000))}k</text>`).join('')}
    ${[0, .25, .5, .75, 1].map(f => `<text x="${xs(Math.round(f * years)).toFixed(1)}" y="${h - pad + 16}" fill="#8a7f74" font-size="10" font-family="'Geist Mono',ui-monospace" text-anchor="middle">${yr + Math.round(f * years)}</text>`).join('')}
...
export function barChart(bars) {
  ...
  return `<svg viewBox="0 0 ${w} ${h}" width="100%" height="${h}" style="display:block">
    ${[0, .25, .5, .75, 1].map(f => `<line x1="${pad}" x2="${w - pad}" y1="${ys(f * max).toFixed(1)}" y2="${ys(f * max).toFixed(1)}" stroke="rgba(36,30,25,.14)" stroke-width="1" ${f > 0 ? 'stroke-dasharray="2 3"' : ''}/>
      <text x="${pad - 6}" y="${(ys(f * max) + 3).toFixed(1)}" fill="#8a7f74" font-size="10" font-family="'Geist Mono',ui-monospace" text-anchor="end">${de0.format(Math.round(f * max / 1000))}k</text>`).join('')}
    ${bars.map((b, i) => `
      ...
      <text x="${(xs(i) + bw / 2).toFixed(1)}" y="${h - pad + 16}" fill="#8a7f74" font-size="10" font-family="'Geist Mono',ui-monospace" text-anchor="middle">${b.label}</text>`).join('')}
  </svg>`;
}
