<script>
  ...
  $: chartSvg = (() => {
    ...
    const cats = [
      { label: 'Fixed Costs', ist: r.fix, emp: r.emp_fix, color: '#e5e5e5' },
      { label: 'Insurance', ist: r.vers, emp: r.emp_vers, color: '#a3a3a3' },
      { label: 'Savings', ist: r.av, emp: r.emp_av, color: '#6fcf97' },
      { label: 'Leisure', ist: r.leb, emp: r.emp_frei, color: '#f2994a' },
      { label: 'Available', ist: r.frei, emp: r.emp_vm, color: '#4a90d9' },
    ];
  ...
  $: legendHtml = `<div style="display:flex;gap:20px;flex-wrap:wrap;margin-top:12px;font-size:11px;font-family:var(--mono);color:var(--fg3)">
    <span style="display:flex;align-items:center;gap:6px"><span style="width:12px;height:12px;background:var(--fg);border-radius:2px;display:inline-block"></span>Actual</span>
    <span style="display:flex;align-items:center;gap:6px"><span style="width:12px;height:12px;background:var(--fg);opacity:.3;border-radius:2px;display:inline-block;border:1px dashed var(--fg)"></span>Recommendation (50/10/15/15 rule)</span>
  </div>`;
</script>
...
<nav class="nav">
  <div class="row g16">
    ...
    <span style="color:var(--fg2);font-size:14px">Result · Cashflow</span>
  </div>
  <div class="row g8">
...
<div class="vscr">
  <div class="calc-result-pad">
    <div class="ey" style="margin-bottom:12px">Cashflow Analysis · Monthly Budget · Actual vs. Recommendation</div>
    <h1 style="font-size:48px;font-weight:600;letter-spacing:-.04em;margin-bottom:28px">
      Available: <span style="color:{r.frei > 0 ? 'var(--fg)' : 'var(--loss)'}">{fmtE(r.frei)}/mo.</span>
    </h1>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);border:1px solid var(--line);border-radius:var(--rlg);overflow:hidden;background:var(--bg1);margin-bottom:20px">
      {#each [['Fixed Costs',fmtE(r.fix),'Recommended: '+fmtE(r.emp_fix)],['Savings',fmtE(r.av),'Recommended: '+fmtE(r.emp_av)],['Leisure',fmtE(r.leb),'Recommended: '+fmtE(r.emp_frei)],['Emergency Fund',fmtE(r.notg),'3 net salaries']] as [l,v,sub], i}
    ...
    <div style="display:grid;grid-template-columns:minmax(0,1fr) 340px;gap:16px">
      <div class="col g16">
        <div class="cardf" style="padding:28px">
          <div class="ey" style="margin-bottom:12px">Actual Expenses vs. Recommendation · Monthly</div>
          {@html chartSvg}
          {@html legendHtml}
        </div>
        ...
        <!-- Detail comparison -->
        <div class="cardf" style="padding:28px">
          <div class="ey" style="margin-bottom:16px">Detailed Comparison · Actual vs. Recommendation</div>
          <div style="display:grid;grid-template-columns:1fr auto auto;gap:0;border-top:1px solid var(--line)">
            <div style="padding:8px 0;font-size:10px;color:var(--fg3);font-family:var(--mono);text-transform:uppercase;letter-spacing:.08em">Category</div>
            <div style="padding:8px 12px;font-size:10px;color:var(--fg3);font-family:var(--mono);text-transform:uppercase;letter-spacing:.08em;text-align:right">Actual</div>
            <div style="padding:8px 0;font-size:10px;color:var(--fg3);font-family:var(--mono);text-transform:uppercase;letter-spacing:.08em;text-align:right">Recommendation</div>
            {#each [
              ['Rent (incl. utilities)',C.miete,''],['Additional Costs',C.nk,''],['Groceries',C.lm,''],['Transport',C.transport,''],['Communication',C.komm,''],
              ['— Total Fixed Costs',r.fix,r.emp_fix,true],
              ['Insurance',C.versich,r.emp_vers],
              ['Retirement Savings',C.av,r.emp_av],
              ['Leisure & Other',r.leb,r.emp_frei],
              ['— Total Expenses',r.out,C.netto * 0.85,true],
              ['Available',r.frei,r.emp_vm,true],
            ] as [l,ist,emp,bold]}
        ...
        <div class="card" style="padding:28px">
          <div class="ey" style="margin-bottom:12px">Emergency Fund Recommendation</div>
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;border-top:1px solid var(--line);padding-top:16px">
            {#each [[3,r.notg,'Minimum'],[6,r.notg*2,'Standard'],[12,r.notg*4,'Comfort']] as [mo,val,l]}
              <div>
          ...
          <p style="font-size:13px;color:var(--fg2);margin-top:16px;line-height:1.6">
            An emergency fund of 3–6 net salaries protects against unexpected expenses (car repairs, dental work, job loss) without going into debt.
          </p>
        </div>
      </div>
      ...
      <aside style="position:sticky;top:88px">
        ...
        <div class="card" style="border-color:var(--fg);position:relative;overflow:hidden">
          ...
          <h3 style="font-size:22px;font-weight:500;margin-bottom:10px">Optimize your cashflow.</h3>
          <p style="font-size:13px;color:var(--fg2);line-height:1.55">Structure your budget, uncover savings potential, build up provisions. 30 min., free.</p>
          ...
          <div style="margin-top:20px;padding-top:16px;border-top:1px solid var(--line)">
            ...
            {#each [['depot','ETF Savings Plan Calculator'],['ruerup','Rürup Calculator'],['versicherung','Insurance Check']] as [k,l]}
